...
enum BoneScanModel
{
	ALONE=0,//单独模式，十倍扫描后，调用此模式
	CONTINUITY,//连续模式，外周血扫描后，调用此模式
};
...
extern "C" _declspec(dllexport)  StatusCode HundredScanMoving(vector<pair<pair<int, int>, pair<int, int>>> points, ScanInfo* scanInfo, HundredScanType hundredScanType,bool ifOil,BoneScanModel boneScanModel);//使用CONTINUITY模式时，外周血和骨髓都是用这个模式
